...
export const nursingRemarks = [
  ["2026/06/25 19:20", "38.2℃。咳嗽・喀痰あり。医師指示にて解熱鎮痛薬使用。"],
  ["2026/06/26 06:40", "夜間，痰が絡む訴えあり。体位調整で軽快。"],
  ["2026/06/27 21:10", "痰が絡む感じでナースコールあり。SpO2 95％。呼吸苦は軽度。"],
  ["2026/06/29 12:40", "昼食後半に疲労感あり。食事時間40分程度。"],
  ["2026/07/01 16:00", "長女面会あり。退院後の生活について「父と協力したい」と話される。"],
  ["2026/07/03 22:40", "夜間，痰が絡む感じで覚醒。体位調整と飲水介助で落ち着く。"]
] as const;
...
export const admissionHistory = [
  "佐藤由紀さんは2年前に筋萎縮性側索硬化症，ALS，上肢型と診断された。発症後から徐々に上肢の筋力低下が進行し，ここ1年ほどは箸の操作や細かな家事が難しくなっていた。半年ほど前からは，食事動作に夫の部分的な介助が必要となり，更衣にも時間を要するようになっていた。歩行はゆっくりであれば自立していたが，疲れやすさが目立っていた。",
...
export const nursingNotes = [
  ...
  },
  {
    ...
    conference: {
      ...
      plan:
        "退院前カンファレンスを2026/07/04 13:30に実施予定。訪問看護週2回，ST週1回の導入を基本案として検討する。PT・OTによる退院後の生活場面評価も必要。デイサービス，福祉用具，自宅環境調整については，家族の意向と支援可能量を確認したうえで検討する。本人，夫，長女の意向と認識に差がないか，退院前カンファレンスで確認する。"
    }
  },
  {
